...
predict_pipeline.py - End-to-end inference:
    raw video  ->  YOLO detection
               ->  interactive per-segment validity check (1 = real throw, 0 = false positive)
               ->  LSTM outcome prediction
               ->  Excel report with an extra "Predicted Outcome" column

Outcome values come from the LSTM (goal / block / out); no manual outcome input.
"""
